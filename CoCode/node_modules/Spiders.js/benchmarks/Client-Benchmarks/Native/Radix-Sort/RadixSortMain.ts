 ...
 */
export class NatRadixSortBench extends SpiderBenchmark{
    sourceRef
    validRef
    allActors
    ...
    runBenchmark(){
        ...
        that.sourceRef.postMessage(["config",BenchConfig.radixSortDataSize,BenchConfig.radixSortMaxVal])
        that.validRef = that.spawnWorker(require('./RadixSortValidation.js'))
        that.validRef.onmessage = sysHandle
        that.validRef.postMessage(["config",BenchConfig.radixSortDataSize])
        var totalActors = 2
        ...
        while(Math.floor(radix) > 0){
            ...
            sortRef.postMessage(["config",BenchConfig.radixSortDataSize,localRadix],[chan.port1])
            radix 				= radix / 2
            totalActors			+= 1
            nextActor 			= sortRef
        }
