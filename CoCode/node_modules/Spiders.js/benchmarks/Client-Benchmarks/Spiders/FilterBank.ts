...
class FilterBankApp extends spiders.Application{
    ...
    setup(){
        ...
        integrateRef.config(BenchConfig.filterBankChannels,combineRef)
        ...
        for(var i = 0;i < BenchConfig.filterBankChannels ; i++){
            ...
            firFilt2Ref.config(i + ".2",BenchConfig.filterBankColumns,taggedRef)
            for(var c in F[i]){
                firFilt2Ref.newCoef(F[i][c])
            }
            firFilt2Ref.configDone()
            var delayRef 		= this.spawnActor(Delay)
            delayRef.config(i + ".2",BenchConfig.filterBankColumns - 1,firFilt2Ref)
            var sampleFiltRef	= this.spawnActor(SampleFilter)
            sampleFiltRef.config(BenchConfig.filterBankColumns,delayRef)
            var firFiltRef 		= this.spawnActor(FirFilter)
            firFiltRef.config(i + ".1",BenchConfig.filterBankColumns,sampleFiltRef)
            for(var c in H[i]){
                firFiltRef.newCoef(H[i][c])
            }
            firFiltRef.configDone()
            var firstRef		= this.spawnActor(Delay)
            firstRef.config(i + ".1",BenchConfig.filterBankColumns - 1,firFiltRef)
            var bankRef 		= this.spawnActor(Bank)
            bankRef.config(i,BenchConfig.filterBankColumns,integrateRef,firstRef)
        ...
        this.producerRef.config(BenchConfig.filterBankSimulations,sourceRef)
    }

    checkConfig() {
        if (this.actorsInitialised == 6 + BenchConfig.filterBankChannels + BenchConfig.filterBankChannels * 6) {
            this.producerRef.nextMessage()
        }
    }
...
export class SpiderFilterBankBench extends SpiderBenchmark{
    filterBankApp
    constructor(){
