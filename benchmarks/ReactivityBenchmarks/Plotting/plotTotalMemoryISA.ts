...
let getMemoryData = (prefix,fileIndex) => {
    ...
    let results = names.map((nodeName)=>{
        return new Promise((resolve)=>{
            var stream          = fss.createReadStream("../GeneralIsa/results/Memory/"+prefix+"Regular"+fileIndex+nodeName+"Memory.csv");
            let allData         = []
            var csvStream = csv()
...
Promise.all(allQPROPResults).then((qpropResults)=>{
    Promise.all(allSIDUPResults).then((sidupResults)=>{
        ...
        plotly.getImage(figure, imgOpts, function (error, imageStream) {
            ...
            var fileStream = fss.createWriteStream('totalMemoryISA.pdf');
