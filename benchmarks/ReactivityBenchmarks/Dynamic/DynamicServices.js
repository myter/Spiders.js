    ...
    checkDynamicLinks() {
        if (this.dynamicLinks.length > 0) {
            setTimeout(() => {
                ...
                for(var i in link){
                    console.log(i + " , " + console.log(link[i]))
                }*/
                let from = link.from;
                let to = link.to;
                console.log("ADDING DYNAMIC DEPENDENCY");
...
class DerivedService extends MicroService_1.MicroServiceApp {
    constructor(isQPROP, rate, totalVals, csvFileName, myAddress, myPort, myTag, directParentsTag, directChildrenTags, dynamicLinkTags) {
        super(myAddress, myPort, exports.monitorIP, exports.monitorPort);
        this.close = false;
        this.rate = rate;
        this.dynamicLinks = dynamicLinkTags;
...
//# sourceMappingURL=DynamicServices.js.map