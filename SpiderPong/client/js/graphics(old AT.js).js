export function game(AmbientJS, roomName, instrumenting) {
	...
	var instrumentOrientation = function() {};
	...
	function Portal() {
		...
		// If we are doing instrumentation, remove randomness!
		if (instrumenting) {
			this.x = 20;
			this.y = 20;
		}
	...
	function ScoreTriangle(i) {
		...
		do {
			// If we are doing instrumentation, remove randomness!
			r1 = (instrumenting) ? (100 + (i*25)) : getRandomInt(this.r, width - this.r);
			r2 = (instrumenting) ? (80  + (i*25)) : getRandomInt(2*pallet.height, height/2);
		}
		...
		this.respawn = function() {
			if(!instrumenting)
				clearTimeout(respawnCountdown);
			delete scoreTriangle;
			scoreTriangle = new ScoreTriangle(i+1);
		};

		// If not hit after 15seconds, spawn a new one (not if we are instrumenting)
		if(!instrumenting) {
			respawnCountdown = setTimeout(this.respawn, 15000);
		}
	}
	...
	function PowerUps() {
		...
		function PowerupSign(x, y, r, c, powerup, powerupName, duration, imagePath) {
			...
			function interceptClick(event) {
				...
				if(dragging && collides({ x: that.x, y: that.y, r: r}, portal, "circle")) {
					...
					var clickedPoint = { x: event.changedTouches[0].pageX, y: event.changedTouches[0].pageY, r: errorMargin }; // We use radius 30 for some error margin (don't need to click exactly on it) 
			...
			function draggingHandler(event) {
				var touchedPoint = { x: event.touches[0].clientX, y: event.touches[0].clientY, r: errorMargin };
			...
			function handleTouchEvents(event) {
				...
				canvas.addEventListener("touchmove", draggingHandler, false);
				canvas.addEventListener("touchend", interceptClick, false);
			}

			function removeListeners() {
				canvas.removeEventListener("touchstart", handleTouchEvents);
				canvas.removeEventListener("touchmove", draggingHandler);
				canvas.removeEventListener("touchend", interceptClick);
			}

			// Set listener to intercept taps and dragging
			canvas.addEventListener("touchstart", handleTouchEvents, false);
		};
		...
		this.multiballPowerup = function() {
			var amount = (instrumenting) ? 5 : getRandomInt(3, 5); // Remove randomness if we are instrumenting
			var originalBall = (balls.length >= 1) ? balls[0] : { x: 50, y: 50, vx: -4, vy: -8};
	...
	function draw() {
		...
		if(instrumenting)
			instrumentOrientation(); // Will simulate the device orientation
	...
	this.setOpponentReference = function(ref) {
		opponent.reference = ref;
		opponent.isPresent = true;
	};
	...
	function sendBall(ball) {
		...
		var teleportedBallMsg = AmbientJS.createMessage("receiveBall", [ball.x, ball.y, ball.vx, ball.vy]);
		opponent.reference.asyncSend(teleportedBallMsg, "oneway");
	...
	function exportScore() {
		if(opponent.isPresent) {
			var scoreMsg = AmbientJS.createMessage("scoreChange", [score.val]);
			opponent.reference.asyncSend(scoreMsg, "oneway");
		}
	}

	function sendPowerup(type) {
		if(opponent.isPresent) {
			var powerupMsg = AmbientJS.createMessage("receivePowerup", [type]);
			opponent.reference.asyncSend(powerupMsg, "oneway");
		}
	}


	/*
	 * Function for automatic instrumentation.
	 * Will do some automated behavior which includes moving the pallet and using powerups.
	 */

	function instrument(devOrientHandler) {
		// Simulates device orientation given an array of directions in degrees and an array of durations for how long (in seconds) to simulate that direction.
		// The last direction will be 
		// 90° is 90° to the right and -90° is 90° to the left.
		function simulateOrientation(directions, durations) {
			var dir, dur;
			if(directions.length > 0 && durations.length > 0) {
				// Get and remove first element of both arrays
				dir = (directions.shift() / -90) * 70; // Convert direction (in degrees) to direction in the range [-70, 70] where -70 equals a device rotated with 90° to the right
				dur = durations.shift() * 1000; 	   // Convert to ms

				instrumentOrientation = function() { devOrientHandler({ beta: dir }); }; // Will be called in each iteration of the gameloop to simulate the device orientation

				setTimeout(function() {
					simulateOrientation(directions, durations);
				}, dur);
			}
			else {
				// Set device orientation to horizontal such that it the pallet stays where it is
				instrumentOrientation = function() { devOrientHandler({ beta: 0 }); };
			}
		}

		// Executes the given type of powerup (locally or remote) after "when" seconds
		function simulatePowerup(type, localOrRemote, when) {
			// In palletPowerup functie zelf die timeout zetten dat die na 20sec terug zich desactiveert want anders desactiveert die zich niet indien het op u tegenstander wordt gedaan
			var invokePowerup;
			var powerup;
			switch(type) {
				case "eBallPowerup":
					powerup 	  = powerups.eBallSign;
					invokePowerup = powerups.extraBallPowerup;
					break;
				case "mBallPowerup":
					powerup 	  = powerups.mBallSign;
					invokePowerup = powerups.multiballPowerup;
					break;
				case "palletPowerup":
					powerup 	  = powerups.palletSign;
					invokePowerup = powerups.palletPowerup;
					break;
			}

			// After "when" seconds --> Invoke powerup locally or send it to opponent, depending on "localOrRemote" argument
			setTimeout(function() {
		...
		// We simulate going to the right, then to the left, staying horizontal and then again go to the left, horizontal and then to the right
		var directions = [0, 5, -4, 0,   5, -7,  7, 0,   -5, 4, -1, 0, -1];
		var durations  = [1, 2,  2, 1.2, 3,  3,  2, 0.5,  2, 5,  2, 1, 1]; // Whole sequence of movements has a duration of 30 seconds
		simulateOrientation(directions, durations);
		simulatePowerup("palletPowerup", "local", 10);
		simulatePowerup("eBallPowerup", "remote", 15);
		simulatePowerup("mBallPowerup", "remote", 20);
	}
}
